...
public function addMenu($request, $response){
  ...
  if($request->isPost()){
      ...
      if($mUploadedFile->getError() === UPLOAD_ERR_OK){
        ...
        $menu->save();
        return $this->view->render($response, 'addmenu.php', array('message' => 'Ajout effectué à la base de données'));
      }else{
...
public function editMenu($request, $response, $args){
  ...
  if($request->isPost()){
    ...
    if($mUploadedFile->getError() === UPLOAD_ERR_OK){
      ...
      return $this->view->render($response, 'editmenu.php', array('message' => 'Ajout effectué à la base de données'));
    }else{

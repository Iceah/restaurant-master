  ...
  public function addMenuItem($request, $response){
    ...
    if($request->isPost()){
      ...
      if($mUploadedFile->getError() === UPLOAD_ERR_OK){
        ...
        $new_menu_item->save();
        return $this->view->render($response, 'addmenuitem.php', array('message' => 'Ajout effectué à la base de données'));
      }else{
   ...
   if($request->isPost()){
     ...
     if(!empty($filename)){
       ...
       MenuItem::where('menu_item_id', $id)->update(['menu_id' => $menu_id, 'item_name' => $item_name, 'item_picture' => $filePath, 'description' => $description,
         ...
         return $this->view->render($response, 'editmenuitem.php', array('message' => 'Ajout effectué à la base de données'));
    }
  }
